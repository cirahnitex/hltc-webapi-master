...
export async function startJob(webapiName: string, numInstances: number):Promise<Job[]> {
    ...
    for(let i=0; i<numInstances; i++) {
        ...
        const qsubCmd = `qsub -o ${packedName}_${WEBAPI_STDOUT_LOG} -e ${packedName}_${WEBAPI_STDERR_LOG} -v PORT=${port} -N ${packedName} -wd ${INSTALLED_WEBAPIS}/${webapiName} ${entryScript}`;
